...
class CsvProcessor(object):
    def __init__(self, fname,
        ...
        self.df = pd.read_csv(fname,
                              usecols=[H_LAST_SWITCHED] + attributes + idx)
    ...
    def get_sparse_array(self, res_small, res_big, old_array=None):
        ...
        if not (old_array is None):  ### Condition to start sparcing from zero or after last array
...
if __name__ == '__main__':
    ...
    # ______________________________________ MAJED __________________________________________
    df_main = pd.read_csv(FNAME)
    dfs = []
    w = 3600 # frame length in seconds
    s = 600 # steps in seconds
    folder_name = f'{w}_Seconds_{s}sec_{Heading}_NoLimit'
    ...
    window_size = pd.Timedelta(seconds=w)
    step_size = pd.Timedelta(seconds=s)
    ...
    for i, df in enumerate(dfs):
        print(f'Shape of Window{i}: {df.shape}')
        df.to_csv(os.path.join(dfs_dir, f'df_{i}.csv'), index=False)

    # ________________________________________ END OF MAJED _________________________________________________________
    ...
    for i,csv_file in tqdm(enumerate(csv_files), desc="Reading CSV files"):
        FNAME = os.path.join(dfs_dir, csv_file)
        ...
        # __________________________________ For another df ________________________________________________________________

        # data_ip = csv_processor_ip.add_new_data(FNAME2, data_ip, RES_SMALL,
        ...
        # ________________________ For the number of features ______________________________________________________________
        ...
        # ________________________ Remooving the zeros _____________________________________________________________________
        ...
        # _________________________ Sklearn ________________________________________________________________________________
