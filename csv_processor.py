...
class CsvProcessor(object):
    def __init__(self, fname, 
        ...
        self.df[H_LAST_SWITCHED] = self.df[H_LAST_SWITCHED].astype('int64')                                                 # I added this line to change dtype of siamak lab files
...
if __name__=='__main__':
    FNAME = REAL_LAB_1a_t1
    FNAME = FNAME[:-4] + '_FIRST2Days' + FNAME[-4:]
    ...
    # data_ip = csv_processor_ip.add_new_data(FNAME2, data_ip, RES_SMALL, RES_BIG)                        ## Read new df and make the first one as a history
    ...
    data_ip = sklearn.preprocessing.scale(data_ip, with_mean=False)                     ## scale the values and with means=false because it is a sparse matrix
