...
class CsvProcessor(object):
    def __init__(self, fname,
        ...
        self.df_labels = pd.read_csv(fname)
        self.attack_timestamps = self.generate_attack_timestamps(self.df_labels,H_Label)
    ...
    def bin_by_resolution(self, res_small,res_big):
        ...
        bins = list(range(self.start_time - res_small,
                          self.end_time + res_small, res_small - (res_small - res_big)))                                        # Here I modify the code o implement shifting
        times = list(range(
    ...
    def generate_attack_timestamps(self,df_,label_col):
        attack_df = df_[df_[label_col] == 1]  # Assuming '1' indicates an attack
        attack_df[H_LAST_SWITCHED] = pd.to_datetime(df_[H_LAST_SWITCHED], unit='s')
        attack_timestamps = attack_df[H_LAST_SWITCHED].unique().tolist()
        return attack_timestamps
...
if __name__ == '__main__':
    ...
    Heading = f'Labelled_{RES_SMALL}S_{RES_BIG}S'
    ...
    utils.plot_transformed_eachwindow_label(transformed_ip,csv_processor_ip.attack_timestamps, csv_processor_ip.start_time,csv_processor_ip.end_time, RES_SMALL, output_dir_ip,consistient_scale,RES_BIG)
